...
public enum HQLRequest {
	...
	DELETE_COMPUTER("DELETE FROM computer WHERE id=?;"), COUNT_COMPUTER("SELECT COUNT(id) AS count FROM computer;"),
	...
	SELECT_ONE_COMPANY("SELECT * FROM company WHERE id=?;"), SELECT_BATCH_COMPANY("SELECT * FROM company LIMIT ?, ?;"),
